...
public class StandardRule implements GameRule {
    ...
    public boolean isValidMove(Board board, int squareId, Player player) {
        if (player.getSide() == PlayerSide.BOTTOM_SIDE) {
            if (squareId < GameConstants.P1_START_INDEX || squareId > GameConstants.P1_END_INDEX) {
                return false;
            }
        } else {
            if (squareId < GameConstants.P2_START_INDEX || squareId > GameConstants.P2_END_INDEX) {
                return false;
            }
        }
